...
// The AdmNumberFunc type is an adapter to allow the use of ordinary
// function as AdmNumber mutator.
type AdmNumberFunc func(context.Context, *ent.AdmNumberMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AdmNumberFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.AdmNumberMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AdmNumberMutation", m)
	}
	return f(ctx, mv)
}
